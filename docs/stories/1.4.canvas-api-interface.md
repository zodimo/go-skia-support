...
- [ ] Task 1: Review C++ SkCanvas Virtual Methods (AC: 5)
  - [ ] Read `skia-source/include/core/SkCanvas.h` lines 2378-2436 (virtual onDraw methods)
  - [ ] Read `skia-source/include/core/SkCanvas.h` lines 2431-2436 (virtual onClip methods)
  - [ ] Document all virtual methods that need to be exposed as interface methods
  - [ ] Identify which methods are essential vs. optional for MVP
  - [ ] Map C++ parameter types to Go types from the library

- [ ] Task 2: Define Core Drawing Primitive Methods (AC: 2)
  - [ ] Create `DrawPaint(paint SkPaint)` method signature (matching C++ `void drawPaint(const SkPaint&)`)
  - [ ] Create `DrawRect(rect Rect, paint SkPaint)` method signature (matching C++ `void drawRect(const SkRect&, const SkPaint&)`)
  - [ ] Create `DrawRRect(rrect RRect, paint SkPaint)` method signature (matching C++ `void drawRRect(const SkRRect&, const SkPaint&)`)
  - [ ] Create `DrawDRRect(outer RRect, inner RRect, paint SkPaint)` method signature (matching C++ `void drawDRRect(const SkRRect&, const SkRRect&, const SkPaint&)`)
  - [ ] Create `DrawOval(oval Rect, paint SkPaint)` method signature (matching C++ `void drawOval(const SkRect&, const SkPaint&)`)
  - [ ] Create `DrawArc(oval Rect, startAngle Scalar, sweepAngle Scalar, useCenter bool, paint SkPaint)` method signature (matching C++ `void drawArc(const SkRect&, SkScalar, SkScalar, bool, const SkPaint&)`)
  - [ ] Create `DrawPath(path SkPath, paint SkPaint)` method signature (matching C++ `void drawPath(const SkPath&, const SkPaint&)`)
  - [ ] Create `DrawPoints(mode PointMode, points []Point, paint SkPaint)` method signature (matching C++ `void drawPoints(PointMode, SkSpan<const SkPoint>, const SkPaint&)`)
  - [ ] Add C++ source references as comments for each method (lines 1287-1584)

- [ ] Task 3: Define Clipping Methods (AC: 3)
  - [ ] Create `ClipRect(rect Rect, op ClipOp, doAntiAlias bool)` method signature (matching C++ `clipRect(const SkRect&, SkClipOp, bool)`)
  - [ ] Create `ClipRRect(rrect RRect, op ClipOp, doAntiAlias bool)` method signature (matching C++ `clipRRect(const SkRRect&, SkClipOp, bool)`)
  - [ ] Create `ClipPath(path SkPath, op ClipOp, doAntiAlias bool)` method signature (matching C++ `clipPath(const SkPath&, SkClipOp, bool)`)
  - [ ] Define `ClipOp` enum (Intersect, Difference, Union, XOR, ReverseDifference) matching C++ `SkClipOp`
  - [ ] Add C++ source references as comments for each method (lines 1019-1151)

- [ ] Task 4: Define Transformation State Methods (AC: 4)
  - [ ] Create `Save() int` method signature (matching C++ `int save()`, returns save count)
  - [ ] Create `Restore()` method signature (matching C++ `void restore()`)
  - [ ] Create `RestoreToCount(saveCount int)` method signature (matching C++ `void restoreToCount(int)`)
  - [ ] Create `GetSaveCount() int` method signature (matching C++ `int getSaveCount() const`)
  - [ ] Create `Concat(matrix SkMatrix)` method signature (matching C++ `void concat(const SkMatrix&)`)
  - [ ] Create `Translate(dx Scalar, dy Scalar)` method signature (matching C++ `void translate(SkScalar, SkScalar)`)
  - [ ] Create `Scale(sx Scalar, sy Scalar)` method signature (matching C++ `void scale(SkScalar, SkScalar)`)
  - [ ] Create `Rotate(degrees Scalar)` method signature (matching C++ `void rotate(SkScalar)`)
  - [ ] Create `Skew(sx Scalar, sy Scalar)` method signature (matching C++ `void skew(SkScalar, SkScalar)`)
  - [ ] Add C++ source references as comments for each method (lines 876-988)

- [ ] Task 5: Create Canvas Interface File (AC: 1, 6, 7, 8, 9, 10)
  - [ ] Create `skia/interfaces/canvas.go` file
  - [ ] Define `SkCanvas` interface with all drawing methods
  - [ ] Define `SkCanvas` interface with all clipping methods
  - [ ] Define `SkCanvas` interface with all transformation methods
  - [ ] Add package-level documentation explaining BYOG philosophy
  - [ ] Add interface-level documentation with usage example
  - [ ] Add method-level documentation with C++ source references
  - [ ] Ensure all types reference existing library types (no new types unless necessary)
  - [ ] Follow Go naming conventions and export rules

- [ ] Task 6: Define Supporting Enums (AC: 3, 4)
  - [ ] Add `ClipOp` enum to `skia/enums/enums.go` (if not exists) - matches C++ `SkClipOp` enum
  - [ ] Add `PointMode` enum to `skia/enums/enums.go` (if not exists) - matches C++ `SkCanvas::PointMode` enum
  - [ ] Reference C++ source: `include/core/SkClipOp.h` and `include/core/SkCanvas.h` (PointMode definition)
  - [ ] Ensure enum values match C++ constants exactly
  - [ ] Note: `doAntiAlias bool` parameter replaces C++ `ClipEdgeStyle` enum (simpler Go API)

- [ ] Task 7: Add Interface Documentation (AC: 7)
  - [ ] Add package doc comment explaining Canvas interface purpose
  - [ ] Add example showing how to implement Canvas interface
  - [ ] Add example showing how to use Canvas with helper functions
  - [ ] Document C++ source file references for each method group
  - [ ] Add note about backend-agnostic design philosophy
