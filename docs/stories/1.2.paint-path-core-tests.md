...
- [ ] Task 1: Port Paint Copy/Equality Tests (AC: 1)
  - [ ] Review `skia-source/tests/PaintTest.cpp` `DEF_TEST(Paint_copy, reporter)` function
  - [ ] Port paint copy/equality test cases to `skia/impl/paint_test.go`
  - [ ] Test paint equality after copy constructor/assignment
  - [ ] Test `Reset()` method returns paint to initial state
  - [ ] Test style, stroke width, mask filter copying
  - [ ] Test paint equality with various property combinations
  - [ ] Verify test results match C++ test expectations

- [ ] Task 2: Port Paint NothingToDraw Tests (AC: 1)
  - [ ] Review `skia-source/tests/PaintTest.cpp` `DEF_TEST(Paint_nothingToDraw, r)` function
  - [ ] Port `nothingToDraw()` test cases to `skia/impl/paint_test.go`
  - [ ] Test default paint: `NothingToDraw()` = false
  - [ ] Test zero alpha: `NothingToDraw()` = true
  - [ ] Test Dst blend mode: `NothingToDraw()` = true
  - [ ] Test color filter that preserves alpha: `NothingToDraw()` = true
  - [ ] Test color filter that modifies alpha: `NothingToDraw()` = false
  - [ ] Verify test results match C++ test expectations

- [ ] Task 3: Port Paint Bounds Computation Tests (AC: 1)
  - [ ] Review `skia-source/tests/PaintTest.cpp` for bounds computation tests
  - [ ] Port paint bounds computation test cases to `skia/impl/paint_test.go`
  - [ ] Test bounds computation with various paint properties
  - [ ] Test edge cases for bounds computation
  - [ ] Verify test results match C++ test expectations (if applicable)

- [ ] Task 4: Port Path Bounds Tests (AC: 2)
  - [ ] Review `skia-source/tests/PathTest.cpp` `test_bounds` function
  - [ ] Port path bounds test cases to `skia/impl/path_test.go`
  - [ ] Test empty path bounds
  - [ ] Test single point bounds
  - [ ] Test line bounds
  - [ ] Test curve bounds
  - [ ] Test multiple contour bounds
  - [ ] Test bounds invalidation after edits
  - [ ] Test `UpdateBoundsCache()` behavior
  - [ ] Verify test results match C++ test expectations
